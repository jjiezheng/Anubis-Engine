...
AVOID BoxTestState::VInitialize( Game * pGame, AUINTPTR uptrData)
{
	...
	EntityResource & sprite_resource = EntityResource("Sprite.xml");
	EntityPtr pBG = pGame->VAddEntity(sprite_resource.VCreateEntity(pGame));
	sprite_resource.VCreateRepresentation(m_pScene, pBG);
	sprite_resource.VCreatePhysicalBody(g_pEngine->GetPhysics(), pBG);
