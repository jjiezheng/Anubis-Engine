...
void GetTileOffsets(in uint tileIdx, out uint startIdx, out uint endIdx)
{
	...
	endIdx = prHigherBound[tileIdx]-1;
}
								 ...
								 in LightParams light)
{
	...
	float3 attenColor = light.color * atten;
...
float4 shading_ps(ps_input input) : SV_TARGET
{
	...
	//return float4((float)numtiles / 81.0, (float)numtiles / 81.0, (float)numtiles / 81.0, 1.0f);
	...
	for (uint lightListIdx = startIdx; lightListIdx <= endIdx; lightListIdx++)
	{
		...
		finalColor += PhysicallyBasedLightning(position, normal, 0.0f, diffuse, specular, specPower, smoothness, directLight);
